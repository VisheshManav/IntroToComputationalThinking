...
function convolve(v::AbstractVector, k)
  n, l = length(v), (length(k) - 1) ÷ 2
  res = fill(0.0, n)
  k_new = OffsetArray(k, -l:l)
  for i in 1:n
    res[i] = [extend(v, i - j) * k_new[j] for j in -l:l] |> sum
  end
  return res
end
...
function convolve(M::AbstractMatrix, K::AbstractMatrix)
  r, c = size(K) .÷ 2
  K_new = OffsetArray(K, -r:r, -c:c)
  convolved = [sum([extend(M, i - m, j - n) * K_new[m, n] for m ∈ -r:r, n ∈ -c:c]) for i ∈ 1:size(M, 1), j ∈ 1:size(M, 2)]
  return convolved
end

# ╔═╡ 6110d541-f6b1-4da5-8fa6-596cfac0c12d
convolve([1, 10, 100], [0.333, 0.333, 0.333])
...
# ╔═╡ d6a9ad97-56ed-455a-8ae5-588bc79054ab
function gaussia_kernel_2D(n; σ=1)
  res = OffsetArray(zeros(2n + 1, 2n + 1), -n:n, -n:n)
  total = 0.0
  for i in -n:n, j in -n:n
    res[i, j] = gauss(i, j, σ=σ)
    total += res[i, j]
  end
  return res ./ total
end
...
function with_sobel_edge_detect(image)
	...
	Gy = Gx'
	x_edges = convolve(image, Gx)
	y_edges = convolve(image, Gy)
	edges = x_edges .^ 2 + y_edges .^ 2 .|> sqrt
    return edges
end
...
# ╔═╡ ea8eb585-dce0-4ed6-be37-8be4d2bfe27a
sobel_camera_image

...
cleaned_sentence_2 = filter(isinalphabet, 
						map(lowercase, messy_sentence_2))
...
first_sample = clean(last(samples))
...
# ╔═╡ 9193ed7e-cfec-4146-b8ff-91c957e9af5e
unused = filter(x->x!="", 
	[(if sample_freqs[i] == 0.0 alphabet[i] else "" end) for i in 1:length(sample_freqs)])
...
# ╔═╡ 8e177438-ba50-44ac-b35e-4c0962f57e2f
begin
	double(x) = x * x
	occur = [occursin(double(i), first_sample) for i in alphabet]
	dletters = filter(i->occur[index_of_letter(i)], alphabet)
end

# ╔═╡ 65c92cac-f930-11ea-20b1-6b8f45b3f262
double_letters = dletters # replace with your answer
...
most_likely_to_follow_w = alphabet[argmax(sample_freq_matrix[index_of_letter('w'), index_of_letter(i)] for i in alphabet)]
...
most_likely_to_precede_w = alphabet[argmax(sample_freq_matrix[index_of_letter(i), index_of_letter('w')] for i in alphabet)]
...
row_col_answer = md"""
	The row_sums denote that the probability of all character after each character
	The col_sums denote the prob. of all character before each character
"""

...
function greedy_seam(energies, starting_pixel::Int)
	m, n = size(energies)
	res = zeros(Int, m)
	clamprow(a) = clamp(a, 1, n)
	res[1] = starting_pixel
	for i in 2:m
		choices = energies[i, clamprow(res[i-1]-1):clamprow(res[i-1]+1)]
		res[i] = argmin(choices) + res[i-1] - 2
		if res[i-1]==1
			res[i] += 1
		end
	end
...
function least_energy(energies, i, j)
	...
	# base case
	if i==m
		return energies[i, clamp(j, 1, n)], j
	end
	## induction
	sw, s, se = least_energy(energies, i+1, clamp(j-1,1,n))[1],
				least_energy(energies, i+1, j)[1],
				least_energy(energies, i+1, clamp(j+1,1,n))[1]
	min_arg = argmin([sw, s, se])+j-2
	if min_arg<1
		min_arg=1
	end
	return energies[i, j] + min(sw, s, se), min_arg
end
...
function recursive_seam(energies, starting_pixel)
	...
	res[1] = starting_pixel
	for i in 2:m
		res[i] = least_energy(energies, i-1, res[i-1])[2]
	end
	res
end
...
exhaustive_observation = md"""
	1).Because from the definition of least_energy() function;
		the recursive seam calculation selects the least_energy neighbour
		where the least_energy() function checks the energy of lower three elements and the elements recursively do the same till the last row, so all paths to the last row are checked to find which has the minimal energy.
		The paths checked forms a pyramid with the starting_index as the head like:
		--*--
		-***-
		***** (Here, all * possible paths with 3 neighbour starting with [1, starting_index] are searched)
	2). if only n inscreases and m remains fixed then the possible paths increase depending on where the starting index is: if starting index is near the start, there are low paths as n increases. else there are more paths; but they still would run out in either case if m is not increased. Eg:
	...
	*** (if increase n by 2)
	...
	--*--
	-***-
	***** (if n increase; no possible addition to number of seams)
...
function memoized_least_energy(energies, i, j, memory::Dict)
	...
	## induction
	sw, s, se = memoized_least_energy(energies, i+1, clamp(j-1,1,n), memory)[1],
				memoized_least_energy(energies, i+1, j, memory)[1],
				memoized_least_energy(energies, i+1, clamp(j+1,1,n), memory)[1]
	min_arg = argmin([sw, s, se])+j-2
	if min_arg<1
		min_arg=1
	end
	memory[(i, j)] = energies[i, j] + min(sw, s, se), min_arg
	return memory[(i, j)]
end
...
function memoized_recursive_seam(energies, starting_pixel)
	...
	res[1] = starting_pixel
	for i in 2:m
		res[i] = memoized_least_energy(energies, i-1, res[i-1], memory)[2]
	end
	res
end
...
function matrix_memoized_least_energy(energies, i, j, memory::Matrix)
	...
	## induction
	sw, s, se = matrix_memoized_least_energy(energies, i+1, clamp(j-1,1,n), memory)[1],
				matrix_memoized_least_energy(energies, i+1, j, memory)[1],
				matrix_memoized_least_energy(energies, i+1, clamp(j+1,1,n), memory)[1]
	min_arg = argmin([sw, s, se])+j-2
	if min_arg<1
		min_arg=1
	end
...
function matrix_memoized_seam(energies, starting_pixel)
	...
	res[1] = starting_pixel
	for i in 2:m
		res[i] = matrix_memoized_least_energy(energies, i-1, res[i-1], memory)[2]
	end
	res
...
function least_energy_matrix(energies)
	...
	for i in m-1:-1:1
		for j in 1:n
			sw = result[i+1, clamp(j-1, 1, n)]
			s  = result[i+1, j]
			se = result[i+1, clamp(j+1, 1, n)]
			result[i, j] = energies[i, j] + min(sw, s, se)
		end
	end
...
function seam_from_precomputed_least_energy(energies, starting_pixel::Int)
	...
	res[1] = starting_pixel
	for i in 2:m
		sw = least_energies[i, clamp(res[i-1]-1, 1 ,n)]
		s  = least_energies[i, res[i-1]]
		se = least_energies[i, clamp(res[i-1]+1, 1 ,n)]
		res[i] = argmin([sw, s, se])+res[i-1]-2
		if res[i] == 0
			res[i] = 1
		end
	end
